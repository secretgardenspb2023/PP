...
## Изменения схемы 2026-06-25 (миграция catalog/0003, по согласованию с заказчицей)

- **`plant.plants.has_author_description`** (boolean, default false) — флаг «авторское
  описание». При индивидуальном добавлении карточки редактором ставится по умолчанию
  (через админ-форму); блокирует будущую массовую перезапись карточки шаблоном (ТЗ №5).
  Существующие донорские карточки — `false`.
- **`dict_colors` перенесён из схемы `plant` в `plant_info`** (к остальным справочникам).
  FK из link-таблиц (`plant_flower_colors`, `plant_leaf_colors`) следуют за таблицей по OID,
  данные/связи сохранены. Модель `Color.db_table = 'plant_info"."dict_colors'`.
- Применено на проде через `manage.py migrate` (RunSQL, идемпотентно), бэкап
  `db-backup-pre-blockC-*.sql.gz`. Шаблон вида: уникальность `is_template` в пределах вида
  поддерживается админ-действием «Переопределить шаблон» (ТЗ №5).
...
Привязана к Django-модели `accounts.User` (`managed=False`) — см. «Решения» ниже.
...
### Привязка `accounts.User` → `user_info.users` (✅ этап 3, миграция `accounts/0003`)

Кастомная модель пользователя (`AbstractBaseUser`, без `PermissionsMixin`) привязана к
существующей клиентской таблице `user_info.users` (`managed = False`, `db_table =
'user_info"."users'`). Логин-идентификатор — `email`. Решение по схеме делегировано исполнителю.

- **Добавлены 4 колонки** (миграция `0003`, `ALTER TABLE`, под `to_regclass`-гард — на тест-БД
  без клиентской схемы no-op): `is_active`, `is_staff`, `is_superuser` (Django-auth/админка не
  работают без них — `is_active` гейтит неподтверждённый email) и `last_login`. Флаги
  **засеяны из `role_id`**: `1 Администратор → is_staff+is_superuser`, `2 Модератор → is_staff`.
- **Маппинг на существующие колонки:** `password` → `password_hash` (varchar 255);
  `date_joined` → `created_at`. `full_name` — property над `first_name`/`last_name`
  (контракт API не меняется).
- **`social_provider`** хранит канонические значения CHECK-констрейнта: `Гугл` (Google),
  `TG` (Telegram), `VK` (на будущее); пусто = `NULL` (а не `''`, иначе CHECK падает).
- **Роли** (`user_info.dict_roles`) маппятся read-only моделью `DictRole`. Новые
  самостоятельно зарегистрированные пользователи получают роль `Пользователь` (id 5);
  `createsuperuser` → `Администратор` (id 1).
- **Права:** Django-группы/`user_permissions` не используются (у клиента своя система ролей);
  доступ к админке — по `is_superuser`/`is_staff` (гранулярные права модераторов — Фаза 2).
- **Удалена** временная Django-managed таблица `accounts_user` (+ её M2M-таблицы).
- **Предусловие миграции на проде/стейдже:** в БД должна быть загружена клиентская схема
  `user_info` (иначе `ALTER` пропускается гардом). Права роли БД: `USAGE` на `user_info` +
  `SELECT/INSERT/UPDATE` на `user_info.users` и `users_id_seq`.
- **VK-вход** — код не написан (нет ключей приложения VK ID), pending-заказчик.
